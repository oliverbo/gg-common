...
describe('extractSong extracts song data', () => {
    it('extracts new song title format (unadorned)', () => {
        const post = wpPost('Artist Name ' + mdash + ' Song Title (ft. Somebody)');
        const song = extractSong(post);
        expect(song?.artist.name).toBe('Artist Name');
        expect(song?.title).toBe('Song Title (ft. Somebody)');
    });

    it('extracts old song pick format', ()=> {
        const post = wpPost('SONG PICK: Artist Name ' + mdash + ' Song Title (ft. Somebody)');
        const song = extractSong(post);
        expect(song?.artist.name).toBe('Artist Name');
        expect(song?.title).toBe('Song Title (ft. Somebody)');
    });
    ...
    it('extracts old song pick + video format', ()=> {
        const post = wpPost('SONG PICK: Artist Name ' + mdash + ' Song Title (ft. Somebody) (Video)');
        const song = extractSong(post);
        expect(song?.artist.name).toBe('Artist Name');
        expect(song?.title).toBe('Song Title (ft. Somebody)');
    });
